...
def plot_diagnosis(axis, y_true, y_pred, threshold, name, dark):
    ...
    sub2 = '{} of {} detected'.format(sum(p_pos > threshold), len(p_pos))
    sub3 = '{}% AUC'.format(auc)
    axis.set_title(name + '\n' + sub1 + ', ' + sub2 + '\n' + sub3)
...
def get_plotter(x, y_true, y_pred, codes, subject_ids, sigs, dark):    
    ...
    def update(code, threshold, example_index, log_scale):
        ...
        for s in range(len(sigs)):
            axes[s+2].clear()
            axes[s+2].plot(x[i, j, :, s], ['b', 'y', 'y', 'y', 'g', 'r'][s])
...
def run(model_id, checkpoint_index, example_count_log2, path, dark=True):
...
#     ])
    ...
    fig, plotter = get_plotter(x_, y_true_, y_pred_, codes, unique_ids, sigs, dark)
